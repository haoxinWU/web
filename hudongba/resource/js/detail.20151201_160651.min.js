    ...
    if (code == 200) {
      ...
      if (recommendLength > 0) {
        ...
        for (var index = 0; index < recommendLength; index++) {
          ...
          recommendHtml += "<li><a class='dt_guess_list party' href='/party/" + recommendItem.infoId36 + "-DInteraction.html'>";
      ...
      if (hotLength > 0) {
        ...
        for (var index = 0; index < hotLength; index++) {
          ...
          hotHtml += "<li><a class='dt_guess_list party' href='/party/" + hotItem.infoId36 + "-DInteraction.html'>";
          //加数据
...
var _postSucess = {
  ...
  _init: function() {
    ...
    if (_info._coverImageUrl != undefined && _info._coverImageUrl != "" && _user._inHudongba && _user._useAndroid && _isCover != 1) {
      setTimeout(function() {
        $("#dt_join_bar").show().animate({"bottom": 0 + "px"}, "slow");
      }, 2000);
    } else {
      setTimeout(function() {
        $("#dt_join_bar").show().animate({"bottom": 0 + "px"}, "slow");
  ...
  if (upEndY < upStartY) {
    if ((upStartY - upEndY) > 100) {
      $(coverDiv).animate({
        ...
        _t._set(_info._type + _info._id + "ifCover", "no");
  ...
  if (downEndY > downStartY) {
    if ((downEndY - downStartY) > 100) {
      $(coverDiv).animate({
        ...
        _t._set(_info._type + _info._id + "ifCover", "yes");
        _coverControl._scrollOn(event);
        _coverControl._init();
      });
    } else {
...
$(document).ready(function() {
  ...
  if (_info._coverImageUrl != undefined && _info._coverImageUrl != "") {
    ...
    var ifCover = _t._get(_info._type + _info._id + "ifCover"),
      y = $(window).height();
    if ($("body").scrollTop() == 0) {
    ...
    if ($("body").scrollTop() == 0) {
      if (ifCover == "" || ifCover == "yes") {
        ...
        $("#coverDiv").animate({
          ...
          $(".content-body,.dt_join_bar,#global_content,#detail_hudongQ").css("opacity", 1);
        });
      } else {
        $("#coverDiv").css("top", -y + "px");
        $(".content-body,.dt_join_bar,#global_content,#coverDiv,#detail_hudongQ").css("opacity", 1);
      }
    } else {
      $("#coverDiv").css("top", -y + "px");
      $(".content-body,.dt_join_bar,#global_content,#coverDiv,#detail_hudongQ").css("opacity", 1);
...
if(_info._type=="find"){
	$(".sclect_ul a").click(function(){//活动筛选
...
var _activiType = { //活动类型选择
  ...
  _select: function(id, name) {
    $(".activity_type").html(name);
    $(".activity_type").attr("id", id);
    _tc._hide("activity_type_tc");
    _activiSelect._post();
  },
  _center: function() {
...
var _activiSelect = { //筛选
  ...
  _post: function() {
    ...
    var _area_id = $("#area_id").children("li").children(".thisOver").attr("id");
    var _area_name = $("#area_id").children("li").children(".thisOver").html();
    var _start_date = $("#start_date").children("li").children(".thisOver").html();
    var _is_charge = $("#is_charge").children("li").children(".thisOver").attr("id");
    var _category_id = $(".activity_type").attr("id");
    var _category_name = $(".activity_type").html();
    var titleName = _area_name + _category_name + "活动_互动吧-最大的活动发布平台";
    dataForShare.title = titleName.replace("_", " ");
    $(document).attr("title", titleName);
    $.ajax({
      type: "post",
      url: _api3._recommendList,
      data: {
        page_size: 20,
        page_num: 1,
...
var _findLoading = {
  ...
  _flag: false,
  _trigger: function() {
    if (_findLoading._flag == false && _._client().h - _._client().bh - 100 <= _._scroll().y) {
      _findLoading._post();
    }
  },
  _init: function() {
  ...
  _post: function() {
    ...
    var _is_charge = $("#is_charge").children("li").children(".thisOver").attr("id");
    var _category_id = $("#category_id span").attr("id");
    var _category_name = $("#category_id span").html();
    if (_findLoading._flag == true) {
      return;
    }
    _findLoading._flag = true;
    _loadingBottom._loading("loading_bottom_find");
    _$(_api3._recommendList, "page_size=20&page_num=" + _findLoading._index + "&app_hdb_id=" + _findLoading._app_hdb_id + "&area_id=" + _area_id + "&start_date=" + _start_date + "&is_charge=" + _is_charge + "&category_id=" + _category_id + "&area_name=" + _area_name + "&category_name=" + _category_name, _findLoading._appendul);
  },
  _ok: function(json, code) {
    var _html = "";
