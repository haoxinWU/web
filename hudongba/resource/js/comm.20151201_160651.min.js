...
var _cookie= {
    _set: function(name, value, expires) {
        ...
        //document.cookie=name+"="+escape(value)+(expires ? (";expires="+_end.toGMTString()) : "")+";path=/;domain=192.168.1.158";
        //document.cookie=name+"="+escape(value)+(expires ? (";expires="+_end.toGMTString()) : "")+";path=/;domain=192.168.1.158";
        document.cookie=name+"="+escape(value)+(expires ? (";expires="+_end.toGMTString()) : "")+";path=/;domain=.hdb.com";
...
if (_user._inHudongba() && _user._useIOs()) {
    var HudongbaJsBridge = {
        ...
        showReviewPanel: function(infoId, infoType, replyUid, replyUName, pId) {
            if(appInfo.app_v >= 4.0) {
                var params = encode.base64Encode("://showReviewPanel::" + infoId + "::" + infoType + "::" + replyUid + "::" + encode._deCode(replyUName) + "::" + pId);
                _g("hudongba" + params);
            } else if (appInfo.app_v >= 3.0) {
        ...
        //详情页精选活动点击更多按钮
        showPostWonderfulPanel:function(type, title, description, voteOption) {
            var params = encode.base64Encode("://showPostWonderfulPanel::" + type + "::" + encode._deCode(title) + "::" + encode._deCode(description) + "::" + encode._deCode(voteOption));
            _g("hudongba" + params);
        },
        //详情页跳转地图
        goMap : function(lon, lat, desc){
            var params = encode.base64Encode("://goMap::" + lon + "::" + lat + "::" + encode._deCode(desc));
            _g("hudongba" + params);
        },
        //通知im
        callim : function(object_id, title, share_image, url){
            var params = encode.base64Encode("://callim::" + object_id + "::" + encode._deCode(title) + "::" + share_image + "::" + url);
            _g("hudongba" + params);
        },
        //跳转首页
        goIndexPage : function(){
            _g("hudongba://goIndexPage");
        },
        //跳发布页
        goPostPage : function(){
            _g("hudongba://goPostPage");
        }
    };
}
...
//        //通知im
//        callim : function(object_id, title, share_image, url){
//            var params = encode.base64Encode("://callim::" + object_id + "::" + encode._deCode(title) + "::" + encode._deCode(share_image) + "::" + encode._deCode(url));
//            _g("hudongba" + params);
//        }
//    };
...
var _loadingBottom= {
    ...
    _loading: function(id) {
        $("#" + id).html("<p><img src='http://img1.hudongba.com/static_v4/images/other/loading2.gif'><span>加载中…</span></p>");
        $("#" + id).unbind("click");
        $("#" + id).show();
    },
    _hide: function(id) {
...
var _setData= function() {
    ...
    if(_user._inHudongba() && _user._useAndroid()){
    	HudongbaJsBridge["setData"](pageTitle, dataForShare.weixin_icon_android, dataForShare.weixin_tl_icon, dataForShare.weixin_url, dataForShare.qq_icon, dataForShare.weibo_icon, dataForShare.url, dataForShare.title, dataForShare.description, dataForShare.sms, _invite, _infoId, _infoType);
    }else{
...
var _login4={
   ...
   _cha:"",
   _post:function(){
	  if(_t._get("partyJoinlogin") == 9){_t._set("partyJoinlogin",10);}
      var _account=_._trim($("#lg_form_account").val()),
	  _password=_._trim($("#lg_form_password").val());
	  if(_account==""){_toast._show("请输入邮箱或手机号");return;}
	  if(!_account.match(/^[\w\.\-]+@[\w\.\-]+\.[a-zA-Z]+$/) && !_account.match(/^1\d{10}$/)){_toast._show("请输入正确的邮箱或手机号");return;}
	  if(_._len(_account)>100){_toast._show("邮箱超出了长度限制");return;}
	  if(_password==""){_toast._show("请输入密码");return;}
	  if(_._len(_password)>16 || _._len(_password)<6){_toast._show("密码请输入6-16个字符");return;}
	  _loading._show("请稍候");
...
var _hdbLoad={
    ...
    _loadimg:function() {
        ...
        for(var i=0; i < _hdbLoad._lazys.length; i++) {
            ...
            if (tmp.x + tmp.w > _hdbLoad._windowOffset.x
                & tmp.x < _hdbLoad._windowOffset.x + _hdbLoad._windowSize.x
                && tmp.y + tmp.h +200> _hdbLoad._windowOffset.y
                && tmp.y < _hdbLoad._windowOffset.y + _hdbLoad._windowSize.y+200) {
                if (tmp.element.getAttribute("data-src") != null) {
                    _hdbLoad._initSize($(tmp.element));
                    $(tmp.element).hide();
                    tmp.element.src = _hdbLoad._lazys[i].datasrc;
    ...
    _initSize:function(self){
        ...
        if(_imgWidth>0){self.css({"width":_imgWidth+"px"});}
    },
    _init:function(){
        _hdbLoad._getLazys();
        _hdbLoad._lazyLoad();
...
var connection = {
		//app端
		check: function(leanUser_object_id,userId){
			var _thisNext = _info._type.replace(_info._type, function($1) {
			    ...
			    if (!_user._login()) {//未登录
			      ...
			      return;
			    	...
			    	if(leanUser_object_id){//如果注册了 id
                        if(_user._inHudongba()){
                            HudongbaJsBridge["callim"](leanUser_object_id, _info._title, dataForShare.share_big_img == "" ? "http://img1.hudongba.com/images/face_default.png" : dataForShare.share_big_img, dataForShare.weixin_url);
                        }
		        	}else{//未注册id，就给注册id
		        		$.post(_api3._leanCloud_chat,{"userId":userId},function(result){
		 	  				var json = eval('(' + result + ')'); 
		 	  				var resultValue=json.object_id;
                            if(_user._inHudongba()){
                                HudongbaJsBridge["callim"](resultValue, _info._title, dataForShare.share_big_img, dataForShare.weixin_url);
                            }
	       			  	});
		        	}
			    }
	    ...
	    connect_pc: function() {
	        $('#tc_downApp4').hide();
	        _tc._show('tc_downApp5');
	    },
	    //mb端wap联系TA
	    checkPlatform: function(leanUser_object_id,userId) {
	    	if(!_user._login()){//判断是否登陆
	    		var _thisNext = _info._type.replace(_info._type, function($1) {
				      return $1 ? ($1.substring(0, 1).toUpperCase() + $1.substring(1)) : '';
				    });
	            if(_user._inHudongba()){_joinBeforeLogin._mark("1");}
	            _user._toLogin(_thisNext);
	            return false;
	        }
	    	function applink(fail) {//salert('进入function');
                return function() {
	    	...
	    	//wap页面party.timeline的联系TA
	        if (/MicroMessenger/i.test(navigator.userAgent)) { //这是微信平台下浏览器
	            if (($('#connection_guide_weixin').length > 0) && ($('#cover_connection').length > 0)) { //如果都有的话
	                $('#connection_guide_weixin, #cover_connection').show();
	                $('#connection_guide_weixin, #cover_connection').click(function() {
	                    $('#connection_guide_weixin, #cover_connection').hide();
	                });
	            }
	        } else if (/(iPhone|iPad|iPod|iOS)/i.test(navigator.userAgent)) { //这是iOS平台下浏览器
	        	var appstore = "http://www.hdb.com/download_redirect";
	        	if(leanUser_object_id){//如果注册了 id
	        		if(_info._type == 'timeline'){
	        			var data = encode.base64Encode(leanUser_object_id + "::" + encode._deCode(dataForShare.weixin_url));
	        		}else if(_info._type == 'party'){
	        			var data = encode.base64Encode(leanUser_object_id + "::" + encode._deCode(_info._title) +"::"+ encode._deCode((dataForShare.share_big_img == "" ? "http://img1.hudongba.com/images/face_default.png" : dataForShare.share_big_img)) + "::" + encode._deCode(dataForShare.weixin_url));
	        		}
	        		window.location = 'hudongbaim://' + data;
	        		applink(appstore)();
	        	
	        	}else{//未注册id，就给注册id
	        		$.post(_api3._leanCloud_chat,{"userId":userId},function(result){
	 	  				var json = eval('(' + result + ')'); 
	 	  				var reslut_value=json.object_id;
	 	  				console.info(json);
	 	  				//网络错误的时候，三个都是undefined；
	 	  				if(_info._type == 'timeline'){
		        			var data = encode.base64Encode(reslut_value + "::" + encode._deCode(dataForShare.weixin_url));
		        		}else if(_info._type == 'party'){
		        			var data = encode.base64Encode(reslut_value + "::" + encode._deCode(_info._title) + "::" + encode._deCode(dataForShare.share_big_img) + "::" + encode._deCode(dataForShare.weixin_url));
		        		}
	        ...
	        } else if (/android/i.test(navigator.userAgent)) { //这是Android平台下浏览器
	            var appstore = "http://www.hdb.com/download?from=post";
	        	if(leanUser_object_id){//如果注册了 id
	        		var data = encode.base64Encode(leanUser_object_id + "::" + encode._deCode(_info._title) +"::"+ encode._deCode((dataForShare.share_big_img == "" ? "http://img1.hudongba.com/images/face_default.png" : dataForShare.share_big_img)) + "::" + encode._deCode(dataForShare.weixin_url));
	        		window.location = 'hudongbaim://' + data;
	        		applink(appstore)();
...
$(document).ready(function() {
  var _cookie_userid_val = _cookie._get("W_U_L_I"); //登录用户的36进制id
  var _cookie_logtoken_val = _cookie._get("W_U_L_S"); //登录令牌
  if (_cookie_userid_val == "" || _cookie_logtoken_val == "") {
    $.ajax({
      type: "get",
      url: _link._fieldCacheHandle,
      data: "",
      dataType: "jsonp",
      jsonp: "jsonpCallback",
      success: function(data) {
        if (data != null) {
          var uid = data.W_U_L_I;
          var uldp = data.W_U_L_S;
          var userpic = data.userpic;
          var username = data.username;
          if ((userpic != undefined && username != undefined) && (userpic != "" || username != "")) {
            $(".mem_l").css("margin-right", 155 + "px");
            $("#a_top_pic").find("img").attr("src", userpic);
            $("#a_top_user").html(username).show();
            $("div.header_public").find(".mem_r").show();
            $("div.header_public").find(".mem_r2").hide();
          } else {
            $("div.header_public").find(".mem_r").hide();
            $("div.header_public").find(".mem_r2").show();
            $(".mem_l").css("margin-right", "0px");
          }
          if (undefined != uid && uid != "") {
            _cookie._set("W_U_L_I", uid, 1000 * 60 * 24 * 365);
          }
          if (undefined != uldp && uldp != "") {
            _cookie._set("W_U_L_S", uldp, 1000 * 60 * 24 * 365);
          }
		  _user._init();
        }
      },
...
//
////=============for avalon, modify by qiutian 20151111================================
//
//var jsonParam = new Object();       //用来获取jsonParam对应的值
//
///**
// * 
// * 获取后台传过来的数据，转换为json格式（jsonParam 在 common_js.jsp中赋值）
// * @param paramKey json数据中的key
// */
//function getJsonParam(paramKey){
//  if(typeof(paramKey) == "undefined"){
//      return jsonParam[default_param_key];  //使用默认值
//  }else{
//      return jsonParam[paramKey];
//  }
//}
//
///**
...
// * @param param 参数值
// */
//function setJsonParam(paramKey,param){
//  jsonParam[paramKey]=param;
//}
//
///**
// * 为json中某个key添加子数据
// * @param {Object} paramKey json数据的key
// * @param {Object} key	    新增子数据的key
// * @param {Object} value   新增子数据值
// */
//function addJsonParamByKey(paramKey,key,value){
//	getJsonParam(paramKey)[key] = value;
//}
//
///**
// * 为json数据添加子数据
...
// * @param {Object} value	新增子数据值
// */
//function addJsonParam(jsonData,key,value){
//	jsonData[key] = value;
//}
////===========================end================================
